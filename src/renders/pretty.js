...
const toString = (obj, level) => {
  if (_.isObject(obj)) {
    ...
    return `${startBlock}${eol}${objStr.join(eol)}${eol}${intend.repeat(level + 1)}${endBlock}`;
  }
  return obj;
};
...
const astToString = (ast, level = 1) => {
  const typeMapping = {
    ...
    }${eol}${makeRow(element.property, toString(element.valueBefore, level), '-', level)}`,
  };
  const outString = ast.map(element => typeMapping[element.type](element, level)).join(eol);
  return `${startBlock}${eol}${outString}${eol}${intend.repeat(level - 1)}${endBlock}`;
};
